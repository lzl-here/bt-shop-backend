...
func reTradeHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		st := s.Stream
		req := new(pay.ReTradeReq)
		if err := st.RecvMsg(req); err != nil {
			return err
		}
		resp, err := handler.(pay.PayService).ReTrade(ctx, req)
		if err != nil {
			return err
		}
		return st.SendMsg(resp)
	case *ReTradeArgs:
		success, err := handler.(pay.PayService).ReTrade(ctx, s.Req)
		if err != nil {
			return err
		}
...
var ReTradeArgs_Req_DEFAULT *pay.ReTradeReq

func (p *ReTradeArgs) GetReq() *pay.ReTradeReq {
	if !p.IsSetReq() {
...
type ReTradeResult struct {
	Success *pay.ReTradeRsp
}

var ReTradeResult_Success_DEFAULT *pay.ReTradeRsp

func (p *ReTradeResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(pay.ReTradeRsp)
	}
	return p.Success.FastRead(buf, _type, number)
}
...
func (p *ReTradeResult) GetSuccess() *pay.ReTradeRsp {
	if !p.IsSetSuccess() {
...
func (p *kClient) ReTrade(ctx context.Context, Req *pay.ReTradeReq) (r *pay.ReTradeRsp, err error) {
