...
func buildRsp(spus []*model.GoodsSpu, skus []*model.GoodsSku, specs []*model.Spec) *ggen.GetGoodsListRsp {
	...
	specRsp := make([]*ggen.GetGoodsListRsp_SpecInfo, 0)
	for _, s := range specs {
		specRsp = append(specRsp, &ggen.GetGoodsListRsp_SpecInfo{
			SpecId:   s.ID,
			SpecName: s.Name,
			SpuId:    s.SpuID,
		})
	...
	for _, spu := range spus {
		rsp.Data.SpuList = append(rsp.Data.SpuList, &ggen.GetGoodsListRsp_SpuInfo{
			SpuId:           spu.ID,
			SpuName:         spu.SpuName,
			SpuDesc:         spu.SpuDesc,
...
func buildSkuList(skus []*model.GoodsSku) []*ggen.GetGoodsListRsp_SkuInfo {
...
func buildAllSpecList(skus []*model.GoodsSku) []*ggen.GetGoodsListRsp_SpecValueInfo {
	rsp := make([]*ggen.GetGoodsListRsp_SpecValueInfo, 0)
	for _, s := range skus {
...
func buildSpecList(specValueStr string) []*ggen.GetGoodsListRsp_SpecValueInfo {
	specValues := new([]*model.SpecValue)
	json.Unmarshal([]byte(specValueStr), specValues)
	rsp := make([]*ggen.GetGoodsListRsp_SpecValueInfo, 0)
	for _, s := range *specValues {
		rsp = append(rsp, &ggen.GetGoodsListRsp_SpecValueInfo{
			SpecValueId: s.ID,
			SpecName:    s.Name,
			SpecValue:   s.Value,
		})
