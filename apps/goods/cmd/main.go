...
func main() {
	...
	flag.Parse()
	// 读取配置文件
	config.AppConfig = config.LoadGoodsConfig(cfgFile)
	// 初始化日志
	...
	// 初始化server
	srv := ggens.NewServer(api.NewGoodsServer(rep),
...
func newRepo(ctx context.Context) *repo.Repo {
	...
	es, err := config.AppConfig.ConnectES(ctx)
	if err != nil {
		panic(err)
	}
	// grpc client
	...
	return repo.NewRepo(db, cache, es, payClient, goodsClient, orderClient, userClient)
}
